...
class BetaZeroParallel:
    def __init__(self, model, optimizer, game: ChessGame, args):
        ...
        self.poolSize = min(self.poolSize, os.cpu_count())  # type: ignore
        ...
        with mp.Pool(self.poolSize) as pool:
            while len(spGames) > 0:
                ...
                for result in pool.map(
                    self.mcts.search,
                    [
                        (states[i : i + step], boards[i : i + step], idx, spGames[i : i + step])
                        for i in range(0, len(spGames), step)
                    ],
                ):
                    actions += result
                ...
                for i in numGames:
                    ...
                    assert all(
                        [
                            all([el1 == el2 for el1, el2 in zip(row1, row2)])
                            for row1, row2 in zip(boardState, stateCheck)
                        ]
                    ), "Board and state different"
                    ...
                    spg.state, spg.board = self.game.getNextState(
                        spg.state, action, spg.board, spg.board.turn == chess.WHITE
                    )
                    ...
                    if isTerminal:
                        ...
                        for turn, tup in enumerate(spg.memory):
                            ...
                            returnMemory.append(
                                (self.game.getEncodedState(histNeutralState), histActionProbs, histOutcome)
                            )
                        del spGames[i]
                    else:
                        spg.state = self.game.changePerspective(spg.state)
    ...
    def learn(self):
        writer = SummaryWriter(f"logdir/{t.time()}")  # TODO: где его вообще нужно импортировать и создавать?
